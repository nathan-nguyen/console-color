...
public class WorldModelGenerator {
  ...
  public void generateWorld() {
    if (!Config.DISABLE_PLAYER) {
      gameContext.modelManager.spawnModel(
          new PlayerModel(gameContext.username, Vector3D.ZERO, true));
    }
  ...
  private void generateWorldBoundary(int startX, int startY, int heightPart, int widthPart) {
    ...
    for (int i = 0; i < heightPart; ++i) {
      Model wbm =
          new WorldBoundaryVerticalModel(
              new Vector3D(
                  startX + i * WorldBoundaryVerticalModel.WORLD_BOUNDARY_PART_HEIGHT, startY, 0),
              true);
      result.add(wbm);
      wbm =
          new WorldBoundaryVerticalModel(
    ...
    for (int i = 0; i < widthPart; ++i) {
      Model wbm =
          new WorldBoundaryHorizontalModel(
              new Vector3D(
                  startX, startY + i * WorldBoundaryHorizontalModel.WORLD_BOUNDARY_PART_WIDTH, 0),
              true);
      result.add(wbm);
      wbm =
          new WorldBoundaryHorizontalModel(
  ...
  private void generateSupportingObject(
      Class<?> clazz, int amount, int startX, int startY, int endX, int endY, Object... args) {
