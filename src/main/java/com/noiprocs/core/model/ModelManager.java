...
public class ModelManager {
  ...
  public void broadcastToClient() {
    try {
      gameContext.networkManager.clientIdMap.forEach(
          (clientId, playerName) -> {
            PlayerModel pm = serverModelManager.playerModelMap.get(playerName);

            Map<String, ModelChunk> chunkMap = new HashMap<>();
            getSurroundedChunk(pm)
                .forEach(modelChunk -> chunkMap.put(modelChunk.getChunkId(), modelChunk));
  ...
  public void addPlayerModel(String playerName) {
    PlayerModel playerModel =
        serverModelManager.playerModelMap.computeIfAbsent(
  ...
  /**
   * Update ServerModelManager from byte array. This method is for Client only.
   *
  ...
  public void update(int dt) {
    ...
    Set<ModelChunk> processChunk =
        gameContext.networkManager.getConnectedPlayerId().stream()
            .flatMap(
                playerName -> {
                  PlayerModel pm = serverModelManager.playerModelMap.get(playerName);
  ...
  private void removeDisconnectedPlayer() {
    List<String> disconnectedPlayer =
        serverModelManager.playerModelMap.keySet().stream()
            .filter(playerName -> !playerName.equals(gameContext.username))
            .collect(Collectors.toList());

    if (Config.DISABLE_PLAYER) disconnectedPlayer.add(gameContext.username);
    disconnectedPlayer.forEach(
        model -> {
  ...
  public Stream<ModelChunk> getLocalChunk() {
    return serverModelManager != null
        ? serverModelManager.chunkMap.values().stream()
        : Stream.empty();
  }
}
