...
public class CotMobModel extends MobModel implements InteractiveInterface {
  ...
  protected void move() {
    ...
    List<Model> surroundedModels =
        hitboxManager.getCollidingModel(this, Vector3D.ZERO, distance, hitboxDimension);
    Optional<AppleItemModel> nearestAppleItemModelOption =
        surroundedModels.stream()
            .filter(model -> model instanceof AppleItemModel)
            .min(
                (u, v) ->
                    Integer.compare(
                        u.position.distanceTo(this.position), v.position.distanceTo(this.position)))
