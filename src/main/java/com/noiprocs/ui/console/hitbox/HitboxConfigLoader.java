...
public class HitboxConfigLoader {
  ...
  private static int parseMaskBit(List<String> masks) {
    return masks.stream().map(HitboxConfigLoader::parseCategoryBit).reduce(0, (a, b) -> a | b);
  }
  ...
  private static int parseDimension(Object value) {
    ...
    if (strValue.startsWith("Config.")) {
      ...
      try {
        ...
        throw new IllegalArgumentException("Failed to resolve Config reference: " + strValue, e);
      }
    }
